...
//		catalogId = "";
	...
	protected Connection returnConnection() throws Exception {
		try {
			...
			return DriverManager.getConnection("jdbc:oracle:thin:@10.16.1.106:1521:VISDB", "VISION_ETL", "Vision123");
		} catch (Exception e) {
			throw e;
		}
	}
...
//		if (tableIdList.size() == 1) {
			if (baseTable != null) {
				if(!tableIdList.contains(String.valueOf(baseTable))) {
					tableIdList.add(String.valueOf(baseTable));
					List<Integer> intTblIdList = tableIdList.stream().mapToInt(e -> Integer.parseInt(e)).boxed().collect(Collectors.toList());
					Collections.sort(intTblIdList);
					tableIdList = new ArrayList<String>();
					tableIdList = intTblIdList.stream().map(v -> String.valueOf(v)).collect(Collectors.toList());
				}
