...
public class CommonApiDao extends AbstractDao<CommonVb>{
	...
	public String getPrdQueryConfig(String queryId) {
		String resultQuery = "";
		try {
			String sql = "SELECT QUERY from PRD_QUERY_CONFIG WHERE DATA_REF_ID = ? AND STATUS = 0 AND APPLICATION_ID = ? ";
			Object[] args = { queryId, productName };
			resultQuery = getJdbcTemplate().queryForObject(sql, args, String.class);
			return resultQuery;
		} catch (Exception ex) {
			return null;
		}
	...
	public int checkSaveTheme(VisionUsersVb vObj) {
		int cnt = 0;
		try {
			String sql = "SELECT  count(1) FROM PRD_APP_THEME where Vision_ID = ? AND APPLICATION_ID = ? ";
			Object[] args = { vObj.getVisionId(), productName };
			cnt = getJdbcTemplate().queryForObject(sql, args, Integer.class);
			if (cnt > 0) {
	...
	public int deleteSaveTheme(VisionUsersVb vObj){
		try
		{			
			String sql = "DELETE FROM PRD_APP_THEME where Vision_ID = ? AND APPLICATION_ID = ? ";
			Object[] args = { vObj.getVisionId(), productName };
