...
public class NumSubTabDao extends AbstractDao<NumSubTabVb> {
	...
	public List findActiveNumSubTabsByNumTabCols(int pNumTab, String cols) throws DataAccessException {
		...
		String sql = "SELECT " + cols
				+ " FROM NUM_SUB_TAB WHERE NUM_SUBTAB_STATUS = 0 AND NUM_TAB = ? ORDER BY NUM_SUB_TAB";
		Object[] lParams = new Object[1];
		lParams[0] = pNumTab;
	...
	public List<NumSubTabVb> findNumSubTabsByNumSubTabs(int pNumTab, int numSubTab) throws DataAccessException {
		String sql = "SELECT * FROM NUM_SUB_TAB WHERE NUM_SUBTAB_STATUS = 0 AND NUM_TAB = ? and NUM_SUB_TAB ! = "
				+ numSubTab + " ORDER BY NUM_SUB_TAB";
		Object[] lParams = new Object[1];
		lParams[0] = pNumTab;
		return getJdbcTemplate().query(sql, lParams, getMapper());
	}
	...
	@SuppressWarnings("unchecked")
	public List<NumSubTabVb> findNumSubTabsByNumTabAndNumSubTab(int pNumTab, int[] numSubTab)
	...
	public List<NumSubTabVb> getQueryPopupResults(NumSubTabVb dObj) {
		...
		StringBuffer strBufApprove = new StringBuffer(
				"Select TAppr.NUM_TAB, TAppr.NUM_SUB_TAB, TAppr.NUM_SUBTAB_DESCRIPTION, TAppr.NUM_SUBTAB_STATUS_NT, "
						+ "TAppr.NUM_SUBTAB_STATUS, TAppr.RECORD_INDICATOR_NT, TAppr.RECORD_INDICATOR,  TAppr.MAKER, TAppr.VERIFIER, "
						...
						+ "') DATE_CREATION, INTERNAL_STATUS From NUM_SUB_TAB TAppr ");
		...
		StringBuffer strBufPending = new StringBuffer(
				"Select TPend.NUM_TAB, TPend.NUM_SUB_TAB, TPend.NUM_SUBTAB_DESCRIPTION, TPend.NUM_SUBTAB_STATUS_NT, "
						...
						+ getDbFunction(Constants.TIME, null)
						+ "') DATE_CREATION, INTERNAL_STATUS From NUM_SUB_TAB_Pend TPend ");
		try {
	...
	public List<NumSubTabVb> getQueryResultsByParent(NumSubTabVb dObj, int intStatus) {
		...
		String strQueryAppr = new String("Select TAppr.NUM_TAB, TAppr.NUM_SUB_TAB, TAppr.NUM_SUBTAB_DESCRIPTION, "
				+ "TAppr.NUM_SUBTAB_STATUS_NT, TAppr.NUM_SUBTAB_STATUS, TAppr.RECORD_INDICATOR_NT, TAppr.RECORD_INDICATOR, "
				+ "TAppr.MAKER, TAppr.VERIFIER, " + getDbFunction(Constants.DATEFUNC, null)
				+ " (TAppr.DATE_LAST_MODIFIED, 'DD-MM-RRRR HH24:MI:SS') DATE_LAST_MODIFIED, " + " "
				+ getDbFunction(Constants.DATEFUNC, null) + " (TAppr.DATE_CREATION, '"
				+ getDbFunction(Constants.DD_Mon_RRRR, null) + " " + getDbFunction(Constants.TIME, null)
				+ "') DATE_CREATION, INTERNAL_STATUS From NUM_SUB_TAB TAppr Where TAppr.NUM_TAB= ?");
		String strQueryPend = new String("Select TPend.NUM_TAB, TPend.NUM_SUB_TAB, TPend.NUM_SUBTAB_DESCRIPTION, "
				+ "TPend.NUM_SUBTAB_STATUS_NT, TPend.NUM_SUBTAB_STATUS, TPend.RECORD_INDICATOR_NT, TPend.RECORD_INDICATOR, "
				+ "TPend.MAKER, TPend.VERIFIER, " + getDbFunction(Constants.DATEFUNC, null)
				+ " (TPend.DATE_LAST_MODIFIED, 'DD-MM-RRRR HH24:MI:SS') DATE_LAST_MODIFIED, " + " "
				+ getDbFunction(Constants.DATEFUNC, null) + " (TPend.DATE_CREATION, '"
				+ getDbFunction(Constants.DD_Mon_RRRR, null) + " " + getDbFunction(Constants.TIME, null)
				+ "') DATE_CREATION, INTERNAL_STATUS From NUM_SUB_TAB_PEND TPend Where TPend.NUM_TAB= ?");
	...
	public void setPendingRecordCount(TabVb objectTemp) {
		...
		StringBuffer strQueryPend = new StringBuffer("Select COUNT('X')"
				+ " From NUM_SUB_TAB_PEND TPend Where TPend.NUM_TAB = ? ");
		Object objParams[] = new Object[intKeyFieldsCount];
	...
	public void setTotRecordCount(TabVb objectTemp) {
		...
		StringBuffer strQueryAppr = new StringBuffer(
				"Select COUNT('X') From NUM_SUB_TAB TAppr Where TAppr.NUM_TAB = ? ");
		Object objParams[] = new Object[intKeyFieldsCount];
		objParams[0] = objectTemp.getTab();// [NUM_TAB]
		try {
			if (!objectTemp.isVerificationRequired()) {
			...
			if (intStatus != 0) {
				int count = getJdbcTemplate().queryForObject(strQueryAppr.toString(), Integer.class, objParams);
	...
	@Override
	protected int getStatus(NumSubTabVb records) {
		return records.getNumSubTabStatus();
	}

	@Override
	protected void setStatus(NumSubTabVb vObject, int status) {
		vObject.setNumSubTabStatus(status);
	}
	...
	protected int doInsertionAppr(NumSubTabVb vObject) {
		String query = "Insert Into NUM_SUB_TAB( "
				...
				+ " DATE_CREATION)  Values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, " + getDbFunction(Constants.SYSDATE, null)
	...
	protected int doUpdateAppr(NumSubTabVb vObject) {
		String query = "Update NUM_SUB_TAB Set "
				...
				+ getDbFunction(Constants.SYSDATE, null) + " Where NUM_TAB = ?  And NUM_SUB_TAB = ? ";

		Object args[] = { vObject.getDescription(), vObject.getNumSubTabStatusNt(), vObject.getNumSubTabStatus(),
				vObject.getRecordIndicatorNt(), vObject.getRecordIndicator(), vObject.getMaker(), vObject.getVerifier(),
				vObject.getInternalStatus(), vObject.getNumTab(), vObject.getNumSubTab() };
	...
	protected int doUpdatePend(NumSubTabVb vObject) {
		String query = "Update NUM_SUB_TAB_PEND Set "
				+ "NUM_SUBTAB_DESCRIPTION = ?, NUM_SUBTAB_STATUS_NT = ?, NUM_SUBTAB_STATUS = ?, RECORD_INDICATOR_NT = ?, "
				+ "RECORD_INDICATOR = ?, MAKER = ?, VERIFIER = ?, INTERNAL_STATUS = ?, DATE_LAST_MODIFIED = "
				+ getDbFunction(Constants.SYSDATE, null) + " Where NUM_TAB = ? "
				+ " And NUM_SUB_TAB = ? ";
	...
	protected int doDeleteAppr(NumSubTabVb vObject) {
		String query = "Delete From NUM_SUB_TAB Where NUM_TAB = ? AND NUM_SUB_TAB = ? ";
	...
	protected int deletePendingRecord(NumSubTabVb vObject) {
		String query = "Delete From NUM_SUB_TAB_PEND Where NUM_TAB = ? AND NUM_SUB_TAB = ? ";
		Object args[] = { vObject.getNumTab(), vObject.getNumSubTab() };
		return getJdbcTemplate().update(query, args);
	}
	...
	public List<NumSubTabVb> findNumSubTabByDesc(String val) throws DataAccessException {
		String sql = "SELECT * FROM NUM_SUB_TAB WHERE NUM_SUBTAB_STATUS = 0 AND NUM_TAB = 2000  and upper(NUM_SUBTAB_DESCRIPTION) "
				+ val + " ORDER BY NUM_SUB_TAB";
	...
	public List<NumSubTabVb> findNumSubTabDesc(String val) throws DataAccessException {
		String sql = "SELECT * FROM NUM_SUB_TAB WHERE NUM_SUBTAB_STATUS = 0 AND NUM_TAB = 1  and upper(NUM_SUBTAB_DESCRIPTION) "
				+ val + " ORDER BY NUM_SUB_TAB";
		return getJdbcTemplate().query(sql, getMapper());
	}

	@SuppressWarnings("unchecked")
	public List<NumSubTabVb> findNumSubTabByCustomFilterAndNumTab(String val, int numTab) throws DataAccessException {
		String sql = "SELECT * FROM NUM_SUB_TAB WHERE NUM_SUBTAB_STATUS = 0 AND NUM_TAB = " + numTab
				+ "  and upper(NUM_SUBTAB_DESCRIPTION) " + val + " ORDER BY NUM_SUB_TAB";
		return getJdbcTemplate().query(sql, getMapper());
	}
