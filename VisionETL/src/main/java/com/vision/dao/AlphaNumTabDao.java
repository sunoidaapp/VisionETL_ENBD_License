...
public class AlphaNumTabDao extends AbstractDao<TabVb> {
	...
	private List<TabVb> getQueryResultsAt(TabVb tabVb, int intStatus) {
		...
		String strQueryAppr = new String(
				"Select ALPHA_TAB, ALPHA_TAB_DESCRIPTION, ALPHA_TAB_STATUS_NT, ALPHA_TAB_STATUS, MAKER, "
						+ "RECORD_INDICATOR_NT, RECORD_INDICATOR, VERIFIER, "
						...
						+ getDbFunction(Constants.DATEFUNC, null) + "(DATE_CREATION, '"
						+ getDbFunction(Constants.DD_Mon_RRRR, null) + " "
						+ getDbFunction(Constants.TIME, null)
						+ "') DATE_CREATION, INTERNAL_STATUS, READ_ONLY From ALPHA_TAB Where ALPHA_TAB = ? ");
		String strQueryPend = new String(
				"Select ALPHA_TAB, ALPHA_TAB_DESCRIPTION, ALPHA_TAB_STATUS_NT, ALPHA_TAB_STATUS, MAKER, "
						...
						+ getDbFunction(Constants.DD_Mon_RRRR, null) + " "
						+ getDbFunction(Constants.TIME, null)
						+ "') DATE_CREATION, INTERNAL_STATUS, READ_ONLY From ALPHA_TAB_PEND Where ALPHA_TAB = ? ");
	...
	private List<TabVb> getQueryResultsNt(TabVb tabVb, int intStatus) {
		...
		setServiceDefaultsNt();
		String strQueryAppr = new String(
				"Select NUM_TAB, NUM_TAB_DESCRIPTION, NUM_TAB_STATUS_NT, NUM_TAB_STATUS, MAKER, "
						+ "RECORD_INDICATOR_NT, RECORD_INDICATOR, VERIFIER, "
						+ getDbFunction(Constants.DATEFUNC, null) + "(DATE_LAST_MODIFIED, '"
						+ getDbFunction(Constants.DD_Mon_RRRR, null) + " "
						+ getDbFunction(Constants.TIME, null) + "') DATE_LAST_MODIFIED, " + ""
						...
						+ getDbFunction(Constants.DD_Mon_RRRR, null) + " "
						+ getDbFunction(Constants.TIME, null)
						+ "') DATE_CREATION, INTERNAL_STATUS, READ_ONLY From NUM_TAB Where NUM_TAB = ? ");
		String strQueryPend = new String(
				"Select NUM_TAB, NUM_TAB_DESCRIPTION, NUM_TAB_STATUS_NT, NUM_TAB_STATUS, MAKER, "
						+ "RECORD_INDICATOR_NT, RECORD_INDICATOR, VERIFIER, "
						+ getDbFunction(Constants.DATEFUNC, null) + "(DATE_LAST_MODIFIED, '"
						...
						+ getDbFunction(Constants.DD_Mon_RRRR, null) + " "
						+ getDbFunction(Constants.TIME, null)
						+ "') DATE_CREATION, INTERNAL_STATUS, READ_ONLY From NUM_TAB_PEND Where NUM_TAB = ? ");
	...
	private List<TabVb> getQueryPopupResultsAt(TabVb tabVb) {
		...
		if(tabVb.isVerificationRequired()){
			...
			// Main Select + Attribute From Cond + (Level Query) + 'Alias Name' + Join Cond
			// + Approve Where Cond + Order
			StringBuffer lApproveQuery = new StringBuffer(
					strNstAppFromCond + " ,(" + strNtApprove + ") X " + strNstApprove);
			CommonUtils.addToQuery(strSubQryJoinCond, lApproveQuery);
			lApproveQuery.append(orderBy);
			if (tabVb.getTotalRows() <= 0) {
				result = paginationhelper.fetchPage(getJdbcTemplate(), lApproveQuery.toString(), objParams,
						tabVb.getCurrentPage(), tabVb.getMaxRecords(), getQueryPopupMapper());
				tabVb.setTotalRows(paginationhelper.getTotalRows());
			} else {
				result = paginationhelper.fetchPage(getJdbcTemplate(), lApproveQuery.toString(), objParams,
						tabVb.getCurrentPage(), tabVb.getMaxRecords(), tabVb.getTotalRows(), getQueryPopupMapper());
			}
		}
		return result;
	}
			...
			for (NumSubTabVb lObject : vObject.getNumSubTabs()) {
				if (lObject.isChecked()) {
					...
					if (exceptionCode.getErrorCode() != Constants.SUCCESSFUL_OPERATION) {
						...
						if (index >= 0) {
							strErrorDesc += " "
									+ exceptionCode.getErrorMsg().replaceFirst("Num Sub Tab - Add - Failed - ", " ");
						} else {
			...
			for (AlphaSubTabVb lObject : vObject.getAlphaSubTabs()) {
				if (lObject.isChecked()) {
					...
					if (exceptionCode.getErrorCode() != Constants.SUCCESSFUL_OPERATION) {
						...
						if (index >= 0) {
							strErrorDesc += " "
									+ exceptionCode.getErrorMsg().replaceFirst("Alpha Sub Tab - Add - Failed - ", " ");
						} else {
							strErrorDesc += " " + exceptionCode.getErrorMsg();
						}
						exceptionCode = getResultObject(Constants.WE_HAVE_ERROR_DESCRIPTION);
...
//		strCurrentOperation = "Delete";
	...
	protected int doUpdateAppr(TabVb vObject) {
		String queryAlpha = "Update ALPHA_TAB Set "
				...
				+ getDbFunction(Constants.SYSDATE, null) + " " + "Where ALPHA_TAB = ? ";
		...
		Object args[] = { vObject.getTabDescription(), vObject.getTabStatusNt(), vObject.getTabStatus(),
				vObject.getRecordIndicatorNt(), vObject.getRecordIndicator(), vObject.getMaker(), vObject.getVerifier(),
				vObject.getInternalStatus(), vObject.getTab() };
		if ("numTab".equalsIgnoreCase(vObject.getRequestType())) {
			return getJdbcTemplate().update(query, args);
		} else {
			return getJdbcTemplate().update(queryAlpha, args);
