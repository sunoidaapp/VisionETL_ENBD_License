...
@Component
public class EtlFeedAlertconfigDao extends AbstractDao<EtlFeedAlertConfigVb> {

	/******* Mapper Start **********/

	String FileStatusNtApprDesc = ValidationUtil.numAlphaTabDescritpionQuery("NT", 1, "TAppr.FEED_ALERT_STATUS",
			"FILE_STATUS_DESC");
	String FileStatusNtPendDesc = ValidationUtil.numAlphaTabDescritpionQuery("NT", 1, "TPend.FEED_ALERT_STATUS",
			"FILE_STATUS_DESC");
	...
	String RecordIndicatorNtUplDesc = ValidationUtil.numAlphaTabDescritpionQuery("NT", 7, "TUPL.RECORD_INDICATOR",
			"RECORD_INDICATOR_DESC");
	...
	protected RowMapper getMapper() {
		RowMapper mapper = new RowMapper() {
			public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
				EtlFeedAlertConfigVb vObject = new EtlFeedAlertConfigVb();
				...
				vObject.setFeedAlertStatusNt(rs.getInt("FEED_ALERT_STATUS_NT"));
				vObject.setFeedAlertStatus(rs.getInt("FEED_ALERT_STATUS"));
	...
	/******* Mapper End **********/
	public List<EtlFeedAlertConfigVb> getQueryPopupResults(EtlFeedAlertConfigVb dObj) {

		Vector<Object> params = new Vector<Object>();
		StringBuffer strBufApprove = new StringBuffer("SELECT TAPPR.COUNTRY, TAPPR.LE_BOOK"
				+ ",TAPPR.FEED_ID,TAPPR.MATRIX_ID,TAPPR.EVENT_TYPE,TAPPR.MATRIX_ID"
				+ ",TAPPR.FEED_ALERT_STATUS_NT,TAPPR.FEED_ALERT_STATUS,TAPPR.RECORD_INDICATOR_NT"
				...
				+ getDbFunction(Constants.DD_Mon_RRRR, null) + " " + getDbFunction(Constants.TIME, null)
				+ "') DATE_CREATION" + " FROM ETL_FEED_ALERT_CONFIG TAPPR ");

		StringBuffer strBufPending = new StringBuffer("SELECT TPEND.COUNTRY, TPEND.LE_BOOK"
				+ ",TPEND.FEED_ID,TPEND.MATRIX_ID,TPEND.EVENT_TYPE,TPEND.MATRIX_ID"
				+ ",TPEND.FEED_ALERT_STATUS_NT,TPEND.FEED_ALERT_STATUS,TPEND.RECORD_INDICATOR_NT"
				...
				+ getDbFunction(Constants.DD_Mon_RRRR, null) + " " + getDbFunction(Constants.TIME, null)
				+ "') DATE_CREATION" + " FROM ETL_FEED_ALERT_CONFIG_PEND TPEND ");
		...
		String strWhereNotExists = new String(
				" NOT EXISTS (SELECT 'X' FROM ETL_FEED_ALERT_CONFIG_PEND TPEND WHERE TAPPR.COUNTRY = TPEND.COUNTRY AND TAPPR.LE_BOOK = TPEND.LE_BOOK AND TAPPR.FEED_ID = TPEND.FEED_ID )");
		try {
	...
	public List<EtlFeedAlertConfigVb> getQueryResults(EtlFeedAlertConfigVb dObj, int intStatus) {
		setServiceDefaults();
		List<EtlFeedAlertConfigVb> collTemp = null;
		...
		StringBuffer strQueryAppr = new StringBuffer("SELECT TAPPR.COUNTRY, TAPPR.LE_BOOK"
				...
				+ getDbFunction(Constants.DD_Mon_RRRR, null) + " " + getDbFunction(Constants.TIME, null)
				+ "') DATE_CREATION"
				+ " FROM ETL_FEED_ALERT_CONFIG TAPPR WHERE UPPER(TAPPR.COUNTRY) = ?  AND UPPER(TAPPR.LE_BOOK) = ?  AND UPPER(TAPPR.FEED_ID) = ?");
		...
		StringBuffer strQueryPend = new StringBuffer("SELECT TPEND.COUNTRY, TPEND.LE_BOOK"
				...
				+ getDbFunction(Constants.DD_Mon_RRRR, null) + " " + getDbFunction(Constants.TIME, null)
				+ "') DATE_CREATION"
				+ " FROM ETL_FEED_ALERT_CONFIG_PEND TPEND WHERE UPPER(TPEND.COUNTRY) = ?  AND UPPER(TPEND.LE_BOOK) = ?  AND UPPER(TPEND.FEED_ID) = ?");

		StringBuffer strQueryUpl = new StringBuffer("SELECT TUPL.COUNTRY, TUPL.LE_BOOK"
				+ ",TUPL.FEED_ID,TUPL.MATRIX_ID,TUPL.EVENT_TYPE,TUPL.MATRIX_ID"
				+ ",TUPL.FEED_ALERT_STATUS_NT,TUPL.FEED_ALERT_STATUS,TUPL.RECORD_INDICATOR_NT"
				+ ",TUPL.RECORD_INDICATOR," + RecordIndicatorNtUplDesc + ",TUPL.MAKER," + makerUplDesc
				+ ",TUPL.VERIFIER," + verifierUplDesc + ",TUPL.INTERNAL_STATUS, "
				...
				+ getDbFunction(Constants.DD_Mon_RRRR, null) + " " + getDbFunction(Constants.TIME, null)
				+ "') DATE_CREATION"
				+ " FROM ETL_FEED_ALERT_CONFIG_UPL TUPL WHERE UPPER(TUPL.COUNTRY) = ?  AND UPPER(TUPL.LE_BOOK) = ?  AND UPPER(TUPL.FEED_ID) = ?");

		if (ValidationUtil.isValid(dObj.getEventType()) && ValidationUtil.isValid(dObj.getMatrixID())) {
			strQueryPend = new StringBuffer(strQueryPend.toString() + " and EVENT_TYPE='" + dObj.getEventType()
					+ "' and MATRIX_ID='" + dObj.getMatrixID() + "' ");
			strQueryAppr = new StringBuffer(strQueryAppr.toString() + " and EVENT_TYPE='" + dObj.getEventType()
					+ "' and MATRIX_ID='" + dObj.getMatrixID() + "' ");
		}
		...
		objParams[2] = dObj.getFeedId().toUpperCase();
	...
	@Override
	protected List<EtlFeedAlertConfigVb> selectApprovedRecord(EtlFeedAlertConfigVb vObject) {
		return getQueryResults(vObject, Constants.STATUS_ZERO);
	}

	@Override
	protected List<EtlFeedAlertConfigVb> doSelectPendingRecord(EtlFeedAlertConfigVb vObject) {
		return getQueryResults(vObject, Constants.STATUS_PENDING);
	}

	@Override
	protected int getStatus(EtlFeedAlertConfigVb records) {
		return records.getFeedAlertStatus();
	}

	@Override
	protected void setStatus(EtlFeedAlertConfigVb vObject, int status) {
		vObject.setFeedAlertStatus(status);
	}

	@Override
	protected int doInsertionAppr(EtlFeedAlertConfigVb vObject) {
		String query = "Insert Into ETL_FEED_ALERT_CONFIG ( COUNTRY, LE_BOOK, FEED_ID, EVENT_TYPE, MATRIX_ID, "
				+ "FEED_ALERT_STATUS, RECORD_INDICATOR_NT, RECORD_INDICATOR, MAKER, VERIFIER, INTERNAL_STATUS, DATE_LAST_MODIFIED, DATE_CREATION)"
				+ "Values (?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?," + getDbFunction(Constants.SYSDATE, null) + ", "
				+ getDbFunction(Constants.SYSDATE, null) + ")";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getFeedId(), vObject.getEventType(),
				vObject.getMatrixID(), vObject.getFeedAlertStatus(), vObject.getRecordIndicatorNt(),
	...
	@Override
	protected int doInsertionUpl(EtlFeedAlertConfigVb vObject) {
		String query = "Insert Into ETL_FEED_ALERT_CONFIG_UPL ( COUNTRY, LE_BOOK, FEED_ID, EVENT_TYPE, MATRIX_ID, "
				+ "FEED_ALERT_STATUS, RECORD_INDICATOR_NT, RECORD_INDICATOR, MAKER, VERIFIER, INTERNAL_STATUS, DATE_LAST_MODIFIED, DATE_CREATION)"
				+ "Values (?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?," + getDbFunction(Constants.SYSDATE, null) + ", "
				+ getDbFunction(Constants.SYSDATE, null) + ")";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getFeedId(), vObject.getEventType(),
	...
	@Override
	protected int doInsertionPend(EtlFeedAlertConfigVb vObject) {
		String query = "Insert Into ETL_FEED_ALERT_CONFIG_PEND ( COUNTRY, LE_BOOK, FEED_ID, EVENT_TYPE, MATRIX_ID, "
				+ "FEED_ALERT_STATUS, RECORD_INDICATOR_NT, RECORD_INDICATOR, MAKER, VERIFIER, INTERNAL_STATUS, DATE_LAST_MODIFIED, DATE_CREATION)"
				+ "Values (?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?," + getDbFunction(Constants.SYSDATE, null) + ", "
				+ getDbFunction(Constants.SYSDATE, null) + ")";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getFeedId(), vObject.getEventType(),
	...
	@Override
	protected int doInsertionPendWithDc(EtlFeedAlertConfigVb vObject) {
		String query = "Insert Into ETL_FEED_ALERT_CONFIG_PEND ( COUNTRY, LE_BOOK, FEED_ID, EVENT_TYPE, MATRIX_ID, "
				+ "FEED_ALERT_STATUS, RECORD_INDICATOR_NT, RECORD_INDICATOR, MAKER, VERIFIER, INTERNAL_STATUS, DATE_LAST_MODIFIED, DATE_CREATION)"
				+ "Values (?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?," + getDbFunction(Constants.SYSDATE, null) + ", "
				+ getDbFunction(Constants.SYSDATE, null) + ")";
	...
	@Override
	protected int doUpdateAppr(EtlFeedAlertConfigVb vObject) {

		String query = "UPDATE ETL_FEED_ALERT_CONFIG SET EVENT_TYPE = ?, MATRIX_ID = ?, "
				+ " FEED_ALERT_STATUS_NT = ?, FEED_ALERT_STATUS = ?, "
		...
		Object[] args = { vObject.getEventType(), vObject.getMatrixID(), vObject.getFeedAlertStatusNt(),
				vObject.getFeedAlertStatus(), vObject.getRecordIndicatorNt(), vObject.getRecordIndicator(),
	...
	@Override
	protected int doUpdatePend(EtlFeedAlertConfigVb vObject) {
		String query = "UPDATE ETL_FEED_ALERT_CONFIG_PEND SET EVENT_TYPE = ?, MATRIX_ID = ?, "
				+ " FEED_ALERT_STATUS_NT = ?, FEED_ALERT_STATUS = ?, "
				+ " RECORD_INDICATOR_NT = ?, RECORD_INDICATOR = ?, MAKER = ?, VERIFIER = ?, INTERNAL_STATUS = ?, DATE_LAST_MODIFIED = "
				+ getDbFunction(Constants.SYSDATE, null) + " WHERE COUNTRY = ?  AND LE_BOOK = ? AND FEED_ID =?";
		Object[] args = { vObject.getEventType(), vObject.getMatrixID(), vObject.getFeedAlertStatusNt(),
	...
	@Override
	protected int doDeleteAppr(EtlFeedAlertConfigVb vObject) {
		String query = "DELETE FROM ETL_FEED_ALERT_CONFIG WHERE COUNTRY = ?  AND LE_BOOK = ? AND FEED_ID=? AND EVENT_TYPE=? AND MATRIX_ID=?";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getFeedId(), vObject.getEventType(),
				vObject.getMatrixID() };
		return getJdbcTemplate().update(query, args);
	}
	...
	@Override
	protected int deletePendingRecord(EtlFeedAlertConfigVb vObject) {
		String query = "DELETE FROM ETL_FEED_ALERT_CONFIG_PEND WHERE COUNTRY = ?  AND LE_BOOK = ? AND FEED_ID=? AND EVENT_TYPE=? AND MATRIX_ID=?";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getFeedId(), vObject.getEventType(),
				vObject.getMatrixID() };
		return getJdbcTemplate().update(query, args);
	}
	...
	public int deleteRecordByPRSParent(EtlFeedMainVb vObject, String tableType) {
		String table = "ETL_FEED_ALERT_CONFIG_PRS";
	...
	protected int doDeleteApprAll(EtlFeedAlertConfigVb vObject) {
		String query = "DELETE FROM ETL_FEED_ALERT_CONFIG WHERE COUNTRY = ?  AND LE_BOOK = ? AND FEED_ID=? ";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getFeedId() };
		return getJdbcTemplate().update(query, args);
	}

	protected int deletePendingRecordAll(EtlFeedAlertConfigVb vObject) {
		String query = "DELETE FROM ETL_FEED_ALERT_CONFIG_PEND WHERE COUNTRY = ?  AND LE_BOOK = ? AND FEED_ID=? ";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getFeedId() };
		return getJdbcTemplate().update(query, args);
	}

	@Override
	protected String getAuditString(EtlFeedAlertConfigVb vObject) {
		...
		strAudit.append("FEED_ALERT_STATUS_NT" + auditDelimiterColVal + vObject.getFeedAlertStatusNt());
		strAudit.append(auditDelimiter);

		strAudit.append("FEED_ALERT_STATUS" + auditDelimiterColVal + vObject.getFeedAlertStatus());
		strAudit.append(auditDelimiter);
	...
	protected void setServiceDefaults() {
		...
		serviceDesc = "ETL Feed Alert Config";
		tableName = "ETL_FEED_ALERT_CONFIG";
		childTableName = "ETL_FEED_ALERT_CONFIG";
	...
	protected ExceptionCode doInsertApprRecordForNonTrans(EtlFeedAlertConfigVb vObject) throws RuntimeCustomException {
		List<EtlFeedAlertConfigVb> collTemp = null;
		...
		if (collTemp.size() > 0) {
			int intStaticDeletionFlag = getStatus(((ArrayList<EtlFeedAlertConfigVb>) collTemp).get(0));
	...
	@Transactional(rollbackForClassName = { "com.vision.exception.RuntimeCustomException" })
	protected ExceptionCode doInsertRecordForNonTrans(EtlFeedAlertConfigVb vObject) throws RuntimeCustomException {
		List<EtlFeedAlertConfigVb> collTemp = null;
		ExceptionCode exceptionCode = null;
		strApproveOperation = Constants.ADD;
		strErrorDesc = "";
		 ...
		 * staticDeletionFlag = getStatus(((ArrayList<EtlFeedAlertConfigVb>)
		...
		if (collTemp.size() > 0) {
			EtlFeedAlertConfigVb vObjectLocal = ((ArrayList<EtlFeedAlertConfigVb>) collTemp).get(0);
	...
	@Transactional(rollbackForClassName = { "com.vision.exception.RuntimeCustomException" })
	protected ExceptionCode doDeleteApprRecordForNonTrans(EtlFeedAlertConfigVb vObject) throws RuntimeCustomException {
		List<EtlFeedAlertConfigVb> collTemp = null;
		ExceptionCode exceptionCode = null;
		...
		setServiceDefaults();
		EtlFeedAlertConfigVb vObjectlocal = null;
		vObject.setMaker(getIntCurrentUserId());
		collTemp = selectApprovedRecord(vObject);
		if (collTemp == null) {
			exceptionCode = getResultObject(Constants.ERRONEOUS_OPERATION);
		...
		vObjectlocal = ((ArrayList<EtlFeedAlertConfigVb>) collTemp).get(0);
...
//			vObject.setRecordIndicator(-1);
	...
	@Transactional(rollbackForClassName = { "com.vision.exception.RuntimeCustomException" })
	public ExceptionCode doRejectRecord(EtlFeedAlertConfigVb vObject) throws RuntimeCustomException {
		EtlFeedAlertConfigVb vObjectlocal = null;
		List<EtlFeedAlertConfigVb> collTemp = null;
		ExceptionCode exceptionCode = null;
		setServiceDefaults();
	...
	@Transactional(rollbackForClassName = { "com.vision.exception.RuntimeCustomException" })
	public ExceptionCode doApproveRecord(EtlFeedAlertConfigVb vObject, boolean staticDelete)
			throws RuntimeCustomException {
		EtlFeedAlertConfigVb oldContents = null;
		EtlFeedAlertConfigVb vObjectlocal = null;
		List<EtlFeedAlertConfigVb> collTemp = null;
		ExceptionCode exceptionCode = null;
		...
		try {
			...
			if (vObjectlocal.getRecordIndicator() != Constants.STATUS_INSERT) {
				...
				oldContents = ((ArrayList<EtlFeedAlertConfigVb>) collTemp).get(0);
			}
...
//				exceptionCode = writeAuditLog(vObjectlocal, oldContents);
	...
	@Transactional(rollbackForClassName = { "com.vision.exception.RuntimeCustomException" })
	protected ExceptionCode doDeleteRecordForNonTrans(EtlFeedAlertConfigVb vObject) throws RuntimeCustomException {
		EtlFeedAlertConfigVb vObjectlocal = null;
		List<EtlFeedAlertConfigVb> collTemp = null;
		ExceptionCode exceptionCode = null;
		strApproveOperation = Constants.DELETE;
	...
	@Transactional(rollbackForClassName = { "com.vision.exception.RuntimeCustomException" })
	protected ExceptionCode doUpdateApprRecordForNonTrans(EtlFeedAlertConfigVb vObject) throws RuntimeCustomException {
		List<EtlFeedAlertConfigVb> collTemp = null;
		EtlFeedAlertConfigVb vObjectlocal = null;
		ExceptionCode exceptionCode = null;
		strApproveOperation = Constants.MODIFY;
	...
	@Transactional(rollbackForClassName = { "com.vision.exception.RuntimeCustomException" })
	protected ExceptionCode doUpdateRecordForNonTrans(EtlFeedAlertConfigVb vObject) throws RuntimeCustomException {
		List<EtlFeedAlertConfigVb> collTemp = null;
		EtlFeedAlertConfigVb vObjectlocal = null;
		ExceptionCode exceptionCode = null;
		strApproveOperation = Constants.MODIFY;
	...
	protected int deleteUplRecordByParent(EtlFeedMainVb vObject) {
		String query = "Delete From ETL_FEED_ALERT_CONFIG_UPL Where COUNTRY = ?  AND LE_BOOK = ?  AND FEED_ID = ? ";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getFeedId() };
		return getJdbcTemplate().update(query, args);
	}

	public int deleteRecordByParent(EtlFeedMainVb vObject, String tableType) {
		String table = "ETL_FEED_ALERT_CONFIG";
		if ("UPL".equalsIgnoreCase(tableType)) {
			table = "ETL_FEED_ALERT_CONFIG_UPL";
		} else if ("PEND".equalsIgnoreCase(tableType)) {
			table = "ETL_FEED_ALERT_CONFIG_PEND";
		}
	...
	public int updateStatusByParent(EtlFeedMainVb vObject, String tableType, int fStatus) {
		...
		String query = "update " + table
				+ " set FEED_ALERT_STATUS = ? ,RECORD_INDICATOR =?  Where COUNTRY = ?  AND LE_BOOK = ?  AND FEED_ID = ?  ";
		Object[] args = { fStatus, vObject.getRecordIndicator(), vObject.getCountry(), vObject.getLeBook(),
				vObject.getFeedId() };
		return getJdbcTemplate().update(query, args);
	}

	public int deleteRecordByParentByCategory(EtlFeedMainVb vObject) {
		String query = "delete FROM ETL_FEED_ALERT_CONFIG_PRS WHERE EXISTS (    SELECT FEED_ID"
				+ "    FROM ETL_FEED_MAIN_PRS    WHERE FEED_CATEGORY = ? AND SESSION_ID = ? "
				+ "	AND ETL_FEED_MAIN_PRS.FEED_ID = ETL_FEED_ALERT_CONFIG_PRS.FEED_ID AND ETL_FEED_MAIN_PRS.SESSION_ID "
				+ " = ETL_FEED_ALERT_CONFIG_PRS.SESSION_ID ) ";
