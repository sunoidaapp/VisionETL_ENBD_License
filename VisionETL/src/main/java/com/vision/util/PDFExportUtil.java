...
public class PDFExportUtil {
	...
	private static PdfPTable createTable(PdfPTable sourceTable, List<PdfPRow> pdfpRows, int colStart, int colEnd,
			int currentPageCount, int totalPages, int labelColCount) {
		...
		if (currentPageCount != totalPages) {
			...
			for (int loopCount = 0; loopCount < pdfpHRows.size(); loopCount++) {
				...
				/*
				 * for(int j=colStart;j<colEnd;j++){ if(cells[j]!=null)
				 * tempTable.addCell(cells[j]); else if(loopCount == 0 && colStart >1 &&
				 * pdfpHRows.size() > 1){ PdfPCell cell = new PdfPCell();
				 * cell.setBackgroundColor(new BaseColor(79, 98, 40)); cell.setBorder(0|0&1|1);
				 * cell.setUseVariableBorders(true); cell.setBorderWidthTop(1);
				 * cell.setBorderWidthBottom(0.5f); cell.setNoWrap(true);
	...
	private static PdfPTable createTableOld(PdfPTable sourceTable, List<PdfPRow> pdfpRows, int colStart, int colEnd,
			int currentPageCount, int totalPages, int labelColCount) {
		PdfPTable tempTable = new PdfPTable((colEnd - colStart) + labelColCount);
		tempTable.setWidthPercentage(99);
		tempTable.setHeaderRows(sourceTable.getHeaderRows());
	...
	static class InvPdfPageEventHelper extends com.itextpdf.text.pdf.PdfPageEventHelper {
		 ...
		 * } catch (IOException e) { // e.printStackTrace(); } catch (DocumentException
		 * e) { // e.printStackTrace(); } }
		 */
		@Override
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			Rectangle footer = new Rectangle(outLine.getLeft(20), outLine.getTop(20), outLine.getRight(20),
					outLine.getBottom(20));
	...
	public ExceptionCode exportToPdfRAWithGroup(List<ColumnHeadersVb> columnHeaders,
			List<HashMap<String, String>> dataLst, ReportsVb reportsVb, String assetFolderUrl, List<String> columnTypes,
			int currentUserId, List<HashMap<String, String>> totalLst,
		...
		try {
			...
			if ("P".equalsIgnoreCase(reportsVb.getReportOrientation())) {
				...
				/*
				 * if(pdfWidthGrwth != 0) { pageWidthSize =
				 * pageWidthSize+(pageWidthSize*pdfWidthGrwth/100); }
				 */
			...
			File lFile = null;
			try {
				InvPdfPageEventHelper invPdfEvt = new InvPdfPageEventHelper(null, reportsVb.getMakerName(),
						assetFolderUrl, new ArrayList(), reportsVb);
								...
								if (type.equalsIgnoreCase(CAP_COL)) {
									...
									if (ValidationUtil.isValid(orgValue)) {
										if ("I".equalsIgnoreCase(colType) || "S".equalsIgnoreCase(colType)) {
											...
											if (ValidationUtil.isNumericDecimal(orgValue)) {
												if (!"NR".equalsIgnoreCase(colType)
														&& !"TR".equalsIgnoreCase(colType)) {
													double amount = Double.parseDouble(orgValue);
	...
	public ExceptionCode exportToPdfRA(List<ColumnHeadersVb> columnHeaders, List<HashMap<String, String>> dataLst,
			ReportsVb reportsVb, String assetFolderUrl, List<String> columnTypes, int currentUserId,
			List<HashMap<String, String>> totalLst, ArrayList<ColumnHeadersVb> columnHeadersFinallst) {
		...
		try {
			...
			try {
					...
					for (ColumnHeadersVb columnHeadersVb : colHeadersCol) {
						...
						if (columnHeadersVb.getColspan() <= 1) {
							if (columnHeadersVb.getNumericColumnNo() != 99) {
								...
								// System.out.println("caption : "+columnHeadersVb.getCaption() +" columnWidth :
								// "+columnWidth);
								colSizes.put(columnHeadersVb.getLabelColNum() - 1,
						...
						for (HashMap dataLstMap : dataLst) {
							...
							for (int loopCount = 0; loopCount < columnHeadersFinallst.size(); loopCount++) {
								...
								if (type.equalsIgnoreCase(CAP_COL)) {
									...
									if (totalRowFlag) {
										ExceptionCode excepCode = writeTotalRowDataColToTable(reportsVb, loopCount,
												cellText, baseFont, table, dataLst, recCount, maxHeaderCol,
												summaryDataFontSize, colHeadersVb, cell);
										if (excepCode.getErrorCode() == Constants.SUCCESSFUL_OPERATION) {
											table = (PdfPTable) excepCode.getResponse();
											cell = (PdfPCell) excepCode.getOtherInfo();
										} else {
										...
										ExceptionCode excepCode = writeRowDataColToTable(reportsVb, loopCount, baseFont,
												table, dataLst, recCount, maxHeaderCol, dataFontSize, formatType,
				...
				float pageHeight = (pageHeightSize)
						- (document.topMargin() + document.bottomMargin() + invPdfEvt.headerHeight + 50);
					...
					if (rowEnd != table.getRows().size()) {
						...
						if (pages > 0) {
							...
							if (reportsVb.getTotalRows() + additionalRows >= rowEnd) {
								...
								tempTable = createTable(table, pdfpRows, maxHeaderCol, table.getNumberOfColumns(),
										pages, totalPages, maxHeaderCol);
								tempTable.setTotalWidth(totalWidth);
								tempTable.setWidthPercentage(percentWidth);
								tempTable.setWidths(colsArry);
								// tempTable.setLockedWidth(true);
	...
	private static float drawHeaderAndFootersRA(Document document, ReportsVb reportsVb, List<PromptIdsVb> prompts,
			String assetFolderUrl) throws MalformedURLException, IOException, DocumentException {
		int height = 30;
	...
	public ExceptionCode exportMultiReportPdf(ReportsVb reportsVb, HashMap<String, ExceptionCode> resultMap,
			String assetFolderUrl) {
		ExceptionCode exceptionCode = new ExceptionCode();
		try {
	...
	public PdfPTable createPromptsPage(Document document, ReportsVb reportsVb)
...
//			cell.disableBorderSide(2);
	...
	private ExceptionCode writeTotalRowDataColToTable(ReportsVb reportsVb, int loopCount, String orgValue,
			BaseFont baseFont, PdfPTable table, List<HashMap<String, String>> dataLst, int recCount, int maxHeaderCol,
			int summaryDataFontSize, ColumnHeadersVb colHeadersVb, PdfPCell cell) {
		String cellText = orgValue;
		ExceptionCode exceptionCode = new ExceptionCode();
		try {
			Font font = new Font(baseFont);
	...
	private ExceptionCode writeRowDataColToTable(ReportsVb reportsVb, int loopCount, BaseFont baseFont, PdfPTable table,
			List<HashMap<String, String>> dataLst, int recCount, int maxHeaderCol, int dataFontSize, String formatType,
			String cellText, ColumnHeadersVb colHeadersVb, PdfPCell cell) {
		ExceptionCode exceptionCode = new ExceptionCode();
		try {
			Font font = new Font(baseFont);
			font.setSize(dataFontSize);
